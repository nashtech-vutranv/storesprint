...
const formateOptions = (options: SearchOption[]) => {
  ...
  for (let k = 0; k < grouppedData.length; k++) {
    for (let j = 0; j < grouppedData[k].length; j++) {
      if (grouppedData[k][j].type === 'users' && count === 0) {
        grouppedData[k].splice(j, 0, {
          label: 'Users',
          value: 'title',
          type: 'title',
        })
      ...
      formattedOptions.push(grouppedData[k][j])
    }
  }
